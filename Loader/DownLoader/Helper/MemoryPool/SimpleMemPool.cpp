...
#define DEFAULT_MEMORY_POOL_SIZE (1024*256)  //默认先分配0.25M
...
namespace DebugerNS
{
    ...
    LPVOID CSimpleMemPool::Alloc(UINT unSize)
    {
        ...
        if (!m_pMem)
        {
            UINT unMemSize = unSize>DEFAULT_MEMORY_POOL_SIZE?unSize:DEFAULT_MEMORY_POOL_SIZE;

            m_pMem = ::HeapAlloc(m_Heap, HEAP_ZERO_MEMORY, unMemSize);	
            if (!m_pMem)
                return NULL;
